...
typedef struct {
    ...
    int STILES_THICK_FIXED_COL_PER_GROUP; // Array for fixed columns per group
...
typedef struct {
    ...
    int *chol_type_per_group;      // dense or sparse chol
    int STILES_NUM_MAX_CORES;       // Maximum number of STILES_NUM_CORES
    int num_chol_indices;         // Total number of Cholesky indices (sum of all STILES_CALLS)
    bool numa_enabled;
    int* global_indicies;
    int** call_matrix;
 ...
 // -----------------------------------------------------------------------------
 int STILES_SETUP_GROUPS(sTiles_object **obj, STILES_CONFIGURATION ***schemes, int num_groups, const int *calls_per_group, const int *cores_per_group, const int *chol_type_per_group,  const bool *get_inverse);

// -----------------------------------------------------------------------------
// Matrix Assignment
// -----------------------------------------------------------------------------
int STILES_ASSIGN_MATRIX_GROUP(int group_index, sTiles_object* stile, 
           int N, int NNZ, int* row_indices, int* col_indices);
int STILES_ASSIGN_MATRIX_CALL(int group_index, int call_index, sTiles_object* stile, int N, int NNZ, int* row_indices, int* col_indices);

// -----------------------------------------------------------------------------
// Preprocessing
// -----------------------------------------------------------------------------
int STILES_PREPROCESS_GROUP(int group_index, sTiles_object **obj, STILES_CONFIGURATION **stiles_schemes);
void STILES_SOLVE_PREPROCESS_CALL(int group_index, int call_index, STILES_CONFIGURATION **schemes, int NG, int* NRHS); 
int STILES_PREPROCESS_CALL(int group_index, int call_index, sTiles_object **obj, STILES_CONFIGURATION **stiles_schemes);

// -----------------------------------------------------------------------------
// Execution: Factorization, Log Determinant, and Inversion
// -----------------------------------------------------------------------------
int STILES_CHOLESKY_CALL(int group_index, int call_index, STILES_CONFIGURATION **schemes);
double STILES_LOGDET_CALL(int group_index, int call_index, STILES_CONFIGURATION **schemes);
int STILES_SELECTED_INVERSE_CALL(int group_index, int call_index, STILES_CONFIGURATION **schemes);

// -----------------------------------------------------------------------------
// Matrix Updates
// -----------------------------------------------------------------------------
int STILES_UPDATE_X_VALUES_CALL(int group_index, int call_index, STILES_CONFIGURATION **stiles_schemes, double *x);

// -----------------------------------------------------------------------------
// Linear System Solving
// -----------------------------------------------------------------------------
int STILES_SOLVE_CALL(int group_index, int call_index, STILES_CONFIGURATION **schemes, double *B, int NRHS, int solve_type, int sindex);

// -----------------------------------------------------------------------------
// Retrieval of Computed Values
// -----------------------------------------------------------------------------
double STILES_GET_SELECTED_INVERSE_ELEMENTWISE_CALL(int group_index, int call_index, int irow, int icol, STILES_CONFIGURATION **scheme);
double STILES_GET_CHOL_VALUE_CALL(int group_index, int call_index, int irow, int icol, STILES_CONFIGURATION **scheme);

// -----------------------------------------------------------------------------
// Initialization and Finalization
// -----------------------------------------------------------------------------
int STILES_INIT_CALL(int group_index, int call_index, sTiles_object** obj);
int STILES_FINILIZE_CALL(int group_index, int call_index, sTiles_object** obj);

// -----------------------------------------------------------------------------
// Memory Management
// -----------------------------------------------------------------------------
void STILES_CLEAN_GROUPS();


int sTiles_create(sTiles_object **obj, int STILES_NUM_CALL_GROUPS, const int *STILES_CALLS_PER_GROUP, const int *STILES_CORES_PER_GROUP, const int *STILES_CHOL_TYPE_PER_GROUP, const bool *STILES_GET_INVERSE);
int sTiles_assign_graph(int group_index, sTiles_object* stile, int N, int NNZ, int* row_indices, int* col_indices);
int sTiles_init(sTiles_object **obj, int NG, int* NRHS);
int sTiles_init_group(int group_index, sTiles_object **obj, int NG, int* NRHS);
