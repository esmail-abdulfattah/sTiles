...
### Mirrors Downloads/Germany4test.R (besag per disease, then a treepcor
### cgeneric kronecker generic0 joint model) but generates a random
### connected "Germany" graph and Poisson data, so it runs anywhere with
### no data files. The inla binary under test comes from INLA_CALL.
...
### Model 1: scaled besag for each disease (as in the original script)
...
### Model 2: treepcor cgeneric + generic0 kronecker (the part that used
### to fail with 'inla_cgeneric_kronecker not found' on stale binaries)
